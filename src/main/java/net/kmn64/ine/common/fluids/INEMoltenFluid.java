...
public class INEMoltenFluid extends FlowingFluid {
	  public static final Collection<INEMoltenFluid> INEMOLTEN_FLUIDS = new ArrayList<>();
	  
	  protected final String fluidName;
	  
	  protected final ResourceLocation stillTex;
	  protected final ResourceLocation flowingTex;
	  protected INEMoltenFluid flowing;
	  protected INEMoltenFluid source;
	  @Nullable
	...
	protected INEMoltenFluid(String fluidName, ResourceLocation stillTex, ResourceLocation flowingTex, @Nullable Consumer<FluidAttributes.Builder> buildAttributes) 
	{ 
		this(fluidName, stillTex, flowingTex, buildAttributes, true); 
	}
	
	protected INEMoltenFluid(String fluidName, ResourceLocation stillTex, ResourceLocation flowingTex, @Nullable Consumer<FluidAttributes.Builder> buildAttributes, boolean isSource) {
	    ...
	    if (!isSource) {
	      ...
	      this.bucket = new BucketItem(() -> this.source, (new Item.Properties()).stacksTo(1).tab(ImmersiveNuclearEngineering.CREATIVE_TAB_MOLTEN).craftRemainder(Items.BUCKET))
	        {
	...
	@OnlyIn(Dist.CLIENT)
	  ...
	  protected INEMoltenFluid createFlowingVariant() {
	    INEMoltenFluid ret = new INEMoltenFluid(this.fluidName, this.stillTex, this.flowingTex, this.buildAttributes, false)
	      {
